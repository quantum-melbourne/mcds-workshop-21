...
def cost_on_classical(coreset_points,order,data_vectors):
    #one = coreset_points[order[0]][1]
    one = [5,3.43,1.46,0.25] # manually calculate the mean point
    other = []
    other.append(coreset_points[order[1]][1])
    other.append(coreset_points[order[2]][1])
    second = np.array([6.2,2.7,4.2,1.3]) # mean of other
    cc = [one,second]
    cost_after_qaoa = 0
    for x in data_vectors:
